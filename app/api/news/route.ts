...
export async function GET(req: NextRequest) {
  try {
    ...
    let articles;
    if (category) {
      articles = await sql`SELECT id, title, slug, category, excerpt, "coverImage", author, "publishedAt" FROM "NewsArticle" WHERE category = ${category} ORDER BY "publishedAt" DESC LIMIT 100`;
    } else {
